...
const initializeDatabase = async (): Promise<void> => {
  ...
  await database.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS idx_restock_requests_sync_status ON restock_requests(sync_status);
    ...
    CREATE INDEX IF NOT EXISTS idx_cycle_counts_product ON cycle_counts(product_id);
    CREATE INDEX IF NOT EXISTS idx_cycle_counts_sync_status ON cycle_counts(sync_status);
...
export const executeSelectOne = async (
  query: string,
  params: any[] = []
): Promise<any | null> => {
  const database = await getDatabase();
